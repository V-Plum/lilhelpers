...
- **Перегляд** — пробіл у Провіднику показує виділений файл, як Quick Look на
  macOS або Peek у PowerToys: зображення (у т. ч. WebP) і SVG, PDF, моделі STL
  із сімома ракурсами, текст і код із підсвіткою, Markdown, docx, форматований
  JSON; відео й аудіо грають просто у вікні перегляду. ⚠ Експериментально,
  з 2.2.0.
- **Знімки екрана** — гаряча клавіша знімає екран, ділянку, вікно або
  зображення з буфера, а редактор дає обвести потрібне, приховати зайве й
  віддати результат у буфер або у файл. Позначки лишаються редагованими:
  знімки зберігаються у власному форматі в бібліотеці, з метаданими EXIF/META.
  З 3.0.0.
- **Запис відео з екрана** — `Alt+Shift+5` записує ділянку, вікно (і йде за
  ним) або екран у MP4 з тією самою HDR-компенсацією, що й знімки; курсор,
  кліки, системний звук і мікрофон — за бажанням. Той самий редактор обрізає й
  вирізає, малює позначки поверх відео, кадрує й зменшує, експортує в MP4 чи GIF
  і зберігає все як проєкт `.lhvideo`. З 4.13.0 — ще й лог DevTools із Chrome/Edge,
  синхронний із відео, і звіт для розробника (відео + лог) одним файлом; з 4.14.0
  запис вікна браузера можна почати й зупинити кліком по значку розширення. З 4.0.0.
...
- **«Не втручатися у вікнах віддалених і віртуальних машин: Caps Lock і
  збільшення курсора»** — Remote Desktop, Windows App, VMware, Hyper-V: хай там
  перемикає (і збільшує курсор) та копія програми, що на віддаленій машині. З
  4.11.0 прапорець керує і збільшенням курсора (CAPS-5): інакше той самий жест
  бачили обидві копії — хост своїм хуком, гість через переслані рухи — і
  зменшення виглядало подвійним. Хост не втручається, коли клієнт активний або
  просто під курсором; без копії програми на гості збільшення в такому вікні
  не буде — це свідома ціна одного вимикача на обидві функції. З 4.9.0 клавіша при цьому не пропускається
  «як є» (тоді хост встигав перемкнути власний Caps Lock — регістр, CAPS-13), а
  пересилається вікну клієнта повідомленням `WM_KEYDOWN`/`WM_KEYUP` з тим самим
  скан-кодом: клієнт передає її в сесію, стан Caps на цій машині не змінюється.
  Чекбокс «Пересилати Caps Lock у вікно клієнта…» вимикає це, якщо якийсь клієнт
  посланого повідомлення не пересилає, — тоді клавіша йде як раніше. У режимі
  «Запасний» хука немає, там поведінка без змін.
...
- **У повноекранних програмах не працює**: `SHQueryUserNotificationState` ловить
  exclusive-D3D і презентаційний режим, (стан «зайнято» — QUNS_BUSY — з 4.10.0 НЕ
  рахується: його тримають RDP-клієнти на всю сесію, і збільшення глухло навіть над
  неактивним розгорнутим вікном — CAPS-6), а borderless-вікна — додаткова перевірка
...
- **Зменшення за замовчуванням малюється копією курсора в layered-вікні**
  ...
  точці), тож опцію можна вимкнути. З 4.12.0 копія чітка (CAPS-4): бітмап
  курсора завжди 32 px, і розтягнутий до 160 px без згладжування він давав
  блоки 5×5. Для стандартних курсорів кадр потрібного розміру береться з
  системного `.cur` (там є до 256 px), для чужих — власний кадр згладжується
  бікубічно; альфа в однобітних курсорів береться з маски, інакше чорна
  стрілка була б невидимою.
...
Дефолти (1200 px шляху, 350 %, 3 зміни напрямку, вікно 700 мс) підібрані на
...
- **Відео** (mp4, mov, avi, wmv, mkv…) — з 4.10.0 справжнє відтворення (CAPS-18):
  той самий Media Engine у режимі frame-server, що й у редакторі відео, але свій
  екземпляр, тож перегляд і редактор живуть одночасно. Стартує сам і **без звуку**
  — перегляд відкривається дуже легко, і гучне відео було б несподіванкою; кнопка
  звуку в смузі під кадром, вибір пам'ятається. У смузі — пуск/пауза, час, доріжка
  (клік і тягнення), звук; клік по самому кадру — пауза/пуск; коліщатко — ±5 с.
  Керування лише мишею: вікно не має фокуса, пробіл і клавіші лишаються
  Провіднику. Перший кадр (роздільність і тривалість у підзаголовку) береться, як
  і раніше, через Source Reader — він же постер, доки рушій не готовий. Два місця,
  де це легко зробити неправильно: без `MF_SOURCE_READER_ENABLE_VIDEO_PROCESSING`
  читач не віддає RGB32 для більшості кодеків, а найперший кадр у багатьох файлах
  чорний — тому перед читанням трохи відмотуємо.
- **Аудіо** (mp3, m4a, aac, wav, flac, ogg, opus, wma…) — з 4.10.0 теж грає, зі
  звуком (тихіше, 50 %), бо мовчазне аудіо не має сенсу; кнопка звуку своя,
  пам'ятається окремо від відео. Картка: значок, назва/виконавець/альбом з тегів
  (через Media Foundation, без сторонніх обробників властивостей), тривалість і
  бітрейт у підзаголовку, та сама смуга керування.
- **Моделі STL** (двійкові й текстові) — власний растеризатор: z-буфер, плоске
  ...
  габарити, які для друку зазвичай і є причиною відкрити файл. **Сім ракурсів
  сторінками** (з 4.11.0, CAPS-55): ізометрія першою — хто не гортає, бачить те,
  що й раніше, — далі спереду, ззаду, зліва, справа, зверху, знизу; коліщатко
  або стрілки в шапці гортають, `Ctrl`+коліщатко масштабує (як у PDF). Ракурс
  малюється при переході на нього і кешується; трикутники живуть, поки картка
  відкрита, і звільняються разом із нею. Це свідомо дешевша альтернатива
  обертанню мишею (CAPS-54): інтерактивному рендеру треба встигати за 16 мс на
  кадр, а сторінці — ні.
...
**Лінза** (з 4.3.0). Прокрутіть коліщатко вгору, і біля перехрестя з'явиться
збільшений фрагмент кадру: ×4, далі ×8 і ×16, без згладжування, із сіткою й
виділеним центральним пікселем. Коліщатко вниз зменшує лінзу, а на ×4 вимикає
її. Під лінзою — координати, розмір рамки й колір пікселя (`#RRGGBB`) зі
зразком. Стрілки зсувають курсор на піксель, з `Shift` — на 10, і під час
тягнення теж. Лінза не закриває ні точку, ні підказку, а біля краю монітора
перескакує на інший бік. Окремого налаштування немає: вибирач завжди
відкривається без лінзи (рішення власника).

**З відліком 3 с** (з 4.3.0) — для меню й підказок, що згортаються від самої
гарячої клавіші. Подвійний клік на вікні або на порожньому місці (увесь
екран), а для ділянки — клік і вдруге тягнути рамку. Накладка зникає, у кутку
монітора йде відлік 3-2-1 (у знімок він не потрапляє), за цей час відкрийте
потрібне меню, і знімається свіжий кадр: вікно подвійного кліку, екран або
ділянка. Модифікатори діють так само, як і без відліку. `Esc` скасовує
відлік. Щоб подвійний клік можна було відрізнити, звичайний клік по вікну
спрацьовує із затримкою до 0,4 с.
...
**Ефекти — тінь і свічення** (з 4.8.0). Кнопка «Ефекти» в смузі відкриває два
рядки по три пресети: тінь — немає, слабка, сильна (м'яка темна, зсунута
вниз-праворуч) і свічення — немає, слабке, сильне (біле сяйво довкола, щоб
позначка «відривалась» від темного тла). Ефект — властивість самої позначки, як
колір чи товщина: міняється на вже намальованій, скасовується одним кроком,
копіюється разом із нею, переживає збереження й потрапляє і в PNG, і у відео
(в програвач і в експорт). Вибране без позначки стає типовим для наступних;
типово ефектів немає. Приховування й маркер ефектів не мають — це дії над
пікселями знімка, а не мазки. Технічно позначка малюється тим самим кодом у
плитку з альфою, з альфи береться маска, зсувається, розмивається й тонується
— і лягає під позначку; зсув і радіус у пікселях знімка, тож файл не залежить
від масштабу на екрані, а тінь не повертається разом із позначкою (світло згори).
...
Версія формату — 1.2 (з 4.8.0): у блоці позначки з'явилися поля ефектів, назви
й «приховано». Вони пишуться лише коли не типові, тож знімок без них байт у байт
такий, як писала 1.1; старіша збірка нові поля пропускає й показує позначки без
ефектів і назв, а файли 1.1 читаються як раніше.

Закриваєте редактор із незбереженими змінами — він питає, зберегти чи ні.

**«Зберегти копію»** (з 4.9.0) у списку біля «Зберегти» кладе в бібліотеку новий
запис із поточним станом — позначками, кадром, тоном, метаданими — під назвою
«<назва> (копія)», далі «(копія 2)»…, і редактор далі працює з копією, як після
«Зберегти як» у звичайних програмах: оригінал лишається таким, як був, а
незбережені правки йдуть у копію. Так з одного знімка робляться варіанти —
скажімо, позначки різних кольорів.
...
Тягнути можна за будь-який елемент: клікнутий стає головним (з 4.12.0,
CAPS-104) — рух рахується від нього, тож стрибка немає, а права панель показує
саме те, за що взялися.
...
**Праві панелі.** З 4.6.0 вони живуть вкладками-значками в смузі праворуч (з 4.8.0
їх три). Відкрита може бути лише одна: клік по іншому значку перемикає панель,
клік по відкритій згортає її. При відкритті редактора всі згорнуті.

- **Властивості** (у відео — «Відео»): звідки знімок, якого розміру, скільки на
  ньому позначок і чи був HDR. Там же поворот на 90° в обидва боки й дзеркало по
  горизонталі й вертикалі: пікселі й позначки їдуть разом, тож стрілка лишається
  на тому, на що показувала.
- **EXIF/META**: назва, опис, автор, авторське право, теги й дата зйомки. Зміна
  фіксується, щойно поле втрачає фокус (Enter, Tab чи клік деінде), і відразу
  пишеться в запис бібліотеки. Дата зйомки — це ще й час самого файлу: і
  «створено», і «змінено». Поля йдуть у файли назовні: у PNG як текстові чанки
  `iTXt`, у JPEG як EXIF (разом із тегами, які показує Провідник), у MP4 як
  метадані `moov/udta/ilst`, які читають програвачі й Провідник. «Прибрати всі
  метадані» очищає поля, і тоді у файли назовні не йде нічого, навіть дата.
  Координат GPS знімки екрана не мають, тож прибирати їх окремо нема чого. Пише
  метадані сама програма, без системних обробників властивостей: їх для
  `.mp4`/`.png` реєструють і сторонні програми, а чужому коду в процесі з правами
  адміністратора не місце. З 4.8.0 у стандартне поле «програма» (PNG `Software`,
  EXIF `Software`, MP4 `©too`) завжди пишеться «Little Helpers x.y.z» — навіть
  коли решта полів порожні; «Прибрати всі метадані» прибирає і його.
- **Позначки** (з 4.8.0): список усіх позначок документа в порядку шарів —
  верхній рядок малюється поверх решти. У рядку значок інструмента, зразок
  кольору, назва, а у відео — ще й проміжок часу. Клік вибирає позначку на
  полотні (у відео — і переходить на кадр, де її видно), вибір на полотні
  підсвічує рядок і прокручує до нього; `Shift`+клік додає до вибору. Рядок
  можна потягнути на інше місце — це змінює порядок шарів, і лінія показує, куди
  він ляже. Назва типово складається з виду й номера («Прямокутник 2», «Напис:
  перші слова», «Лічильник 3»); подвійний клік або `F2` перейменовує на місці,
  `Enter` фіксує, `Esc` скасовує, порожня назва повертає автоматичну. Око
  праворуч ховає позначку: вона не малюється, не ловиться мишею й не йде в
  експорт; ще одним кліком — назад. Права кнопка на рядку — меню: перейменувати,
  дублювати, на передній чи задній план, приховати, видалити. Кожна дія — один
  крок скасування. Назви й «приховано» зберігаються в `.lhshot` і `.lhvideo`.
  **Група — один вузол** (з 4.10.0, CAPS-100): «Група N · k» зі вкладеними рядками,
  згортається трикутником; тягнеться лише цілком, а всередину нічого не
  вставиш — лінія відскакує до межі вузла; клік, око, назва (подвійний клік) і
  меню на вузлі діють на всю групу, там же — «Розгрупувати». При групуванні
  учасники стягуються докупи в z-порядку під найвищим із них (так само при
  відкритті старих файлів), інакше вузол обіцяв би порядок малювання, якого
  немає. Назви груп — у файлі (блок `GRPN`, формат 1.3 / 1.2).
...
час запису першим пунктом меню — «Зупинити запис» і «Пауза запису».

З 4.11.0 біля правого нижнього кута рамки стоїть **плашка «пауза / стоп»**
(CAPS-101): пауза (кнопка міняється на «продовжити»), стоп (те саме, що клавіша)
і лічильник часу без пауз. Стоїть зовні ділянки, щоб не потрапляти в кадр; нема
місця знизу — стає зверху, нема ніде (весь екран) — всередині, але з захоплення
вона виключена завжди, як і рамка. Це окреме клікабельне вікно без фокуса
(рамка пропускає мишу наскрізь), кліки по ньому не потрапляють у запис як кліки
курсора. На паузі рамка й плашка жовті, лічильник мигає — щоб не забути, що
запис стоїть. Пауза — саме пауза, а не «дірка»: кадри не пишуться, після
продовження час зсувається на тривалість паузи, тож у файлі ні стрибка, ні
замороженого кадру; звук так само — записане під час паузи викидається, і
доріжка лишається в такт кадрам. Клавіші для паузи навмисно немає (рішення
власника 26.09): лише кнопка й трей. У режимі «слідувати за вікном» плашка їде
за рамкою і ховається зі згорнутим вікном.
...
значку трею — червона крапка, у підказці — тривалість. Після стопу запис сам
відкривається в редакторі (з 4.4.1). Лише якщо в редакторі є незбережена робота,
його не чіпають: тоді біля курсора з'являється «Відео збережено — клацніть, щоб
відкрити».

**Запис вікна (з 4.5.0).** Клік по вікну в рамці вибору записує **саме вікно**.
У кадр потрапляє лише воно: вікна, що його перекривають, не видно, а якщо його
пересунути, навіть на інший монітор, запис іде за ним. Червона рамка-індикатор
теж їде за вікном. Розмір відео — розмір вікна на старті. Якщо вікно змінить
розмір, кадр вписується по центру з полями, без спотворення пропорцій; менше
вікно не розтягується. Згорнуте вікно кадрів не дає, тож у відео лишається
останній кадр. Закрили вікно — запис зупиняється сам і зберігається. Колір і
рівень білого беруться з монітора, де вікно зараз, тож вікно, пересунуте між
HDR- і SDR-монітором, лишається правильно експонованим. На вкладці «Відео»
«Клік по вікну: ділянкою» повертає давню поведінку: пишеться прямокутник
екрана, де вікно було на старті. Захищений вміст (DRM-відео, вікна, закриті від
захоплення) приходить чорним: так його віддає сама Windows. Потрібна Windows 10
1903 або новіша, на старіших вікно пишеться ділянкою.

Як пишеться: Windows.Graphics.Capture, кадр одразу у scRGB (FP16). Далі той
самий шейдер тону, що й для екрана, плюс вписування. Жовту рамку, якою Windows
позначає захоплення, і системний курсор вимкнено: курсор малюємо самі. Тестовий
харнес записує власне вікно справжніми кадрами: його рухають, перекривають,
звужують, згортають і закривають, а декодований MP4 показує в кожній фазі саме
його кольори.
...
**Курсор і кліки** (з 4.3.0, обидва типово ввімкнені). Екран віддає кадр без
курсора, тож курсор домальовується тим самим шейдером, уже після тону: на
HDR-екрані білий курсор лишається білим. Текстовий курсор, що інвертує фон,
інвертує його й у відео, тож на темному фоні він не зникає. Клік — жовта
(червона, синя) точка з колом, що розходиться ~350 мс, правий клік — подвійне
коло, а поки кнопку тримають (тягнуть), під курсором лишається кільце. Кліки
приходять із хука миші з тими самими мітками часу, що й кадри, тож коло
з'являється в кадрі самого кліку. Журнал кліків лягає в `.lhmeta` для редактора
й звіту далі.

**Звук** (з 4.3.0, типово вимкнений). На вкладці «Відео» — «Системний звук»
(те, що чути в колонках чи навушниках) і «Мікрофон», кожен зі своїм пристроєм
(типовий або вибраний); смужка під мікрофоном показує, що він чує. Обидва
джерела змішуються в одну доріжку AAC 48 кГц: більшість програвачів і
месенджерів грають лише першу. Кожен пакет звуку лягає на шкалу за своєю
міткою часу, тож паузи (коли нічого не грає, системний звук мовчить зовсім)
стають тишею, а не зсувають доріжку; витягнули навушники — запис іде далі
тишею й підхоплює новий типовий пристрій. Якщо мікрофон заборонено в
параметрах конфіденційності Windows чи пристрій зайнятий, запис іде без нього,
а в треї з'являється пояснення.
...
30/60 кадрів, якість («Менша», «Звичайна», «Висока»), що записує клік по вікну
(слідувати за ним чи ділянкою), курсор і кліки, звук, межа
обсягу відео в бібліотеці (типово 5 ГБ). Межа **окрема від знімків**: понад неї найстаріші
...
правій панелі — відомості й «Відкрити кадр як знімок». Рейка та сама, і її
інструменти малюють позначки поверх відео (див. нижче). «Копіювати»
...
відео саме після запису, з бібліотеки, через «Відкрити…» і
...
### Обрізати, вирізати, зберегти (з 4.2.0)
...
**«Зберегти»** (`Ctrl+S`) з 4.4.0 зберігає правки як проєкт `.lhvideo`, див.
нижче. У 4.2.0 і 4.3.0 воно клало в бібліотеку новий запис із позначкою
«(обрізано)». Готовий MP4 — у списку поруч:

- «Зберегти як…» — MP4 у вибрану теку;
- «Копіювати як файл» — у буфер як файл, тож можна вставити в месенджер, лист
  або теку;
- «Зберегти копію» (з 4.9.0) — новий проєкт `.lhvideo` у бібліотеці з тим самим
  відео байт у байт і поточним станом (правки, позначки, кадр), назва
  «(копія)»; далі редактор працює з копією, оригінал не чіпається.
...
кожен кадр синтетики, має йти рівно за відрізками.

### Позначки поверх відео (з 4.4.0)

Ті самі інструменти, що й для знімків: рамка, еліпс, лінія зі стрілками,
олівець, напис, приховування, маркер, лічильник, штамп і зображення. Кадрування
у відео одне на весь запис — див. «Кадрувати й зменшити відео». Поставте відео
на паузу, виберіть інструмент і малюйте на кадрі.
Позначка з'являється з цього кадру на 3 с, а на доріжці під стрічкою з'являється
її смуга. Смугу можна тягнути, щоб пересунути позначку в часі, або тягнути за
край, щоб змінити, коли вона видна. Під курсором смуга підсвічується, на її
краях з'являються ручки, а курсор показує, що зараз буде: «↔» над краєм,
«пересунути» над серединою. Клік по смузі вибирає позначку.

Позначки, що перетинаються в часі, лягають на окремі доріжки, і смуги ніколи не
накладаються. Видно до чотирьох доріжок. Решта прокручується коліщатком над
доріжками, а праворуч тоді з'являється тонка смуга прокрутки. Вище вікно або
дрібніше відео дає місце для більшої кількості доріжок: у режимі відео масштаб
можна зменшити до половини від «Вписати».

- **Редагування.** На паузі поточний кадр стає зображенням редактора, тож
  працює все звичне: вибір, ручки, кольори, товщина, порядок, дублювання і
  `Delete`. Видно й доступні лише позначки, живі на цьому кадрі. З інструментом
  малювання чи вибраною позначкою смуга під заголовком показує властивості
  позначки, інакше — відтворення.
- **Під час відтворення** позначки малює прозорий шар поверх відео.
  Приховування в ньому — заштрихована плашка, бо розмивати кожен кадр наживо
  надто дорого. Справжнє розмиття видно на паузі й у файлі.
- **Скасування** одне на все: і правки таймлайну, і позначки.
- **«Зберегти як…» і «Копіювати як файл»** малюють позначки в кожен кадр
  їхнього проміжку. Приховування
  рахується з пікселів під ним на кожному кадрі, тими самими функціями, що й у
  редакторі знімків, тож розмиття «живе», а не заморожене. «Копіювати» і
  «Відкрити кадр як знімок» беруть кадр разом із позначками; у знімок вони
  переходять редагованими.

### Проєкт відео `.lhvideo` (з 4.4.0)

Якщо є правки таймлайну чи позначки, «Зберегти» кладе в бібліотеку проєкт замість
перекодованого MP4. Це відео разом з усім, що з ним зроблено: вирізане, ручки,
позначки, назва, журнал миші. Відкритий знову з бібліотеки, проєкт відновлює все
для редагування. Старий запис MP4 іде в кошик, щойно відео закриють. Уже
відкритий проєкт «Зберегти» оновлює на місці й одразу.

Готовий файл для передачі — «Зберегти як…» (MP4 у вибрану теку) або «Копіювати
як файл». Вирізане там уже прибрано, а позначки впечені в кадри.

**Як.** Проєкт — один файл: записане MP4 байт у байт, а після нього MP4-блок
`uuid` із правками. Невідомі блоки програвачі пропускають, тож `.lhvideo`,
перейменований на `.mp4`, грає будь-який програвач: оригінальне відео, без
правок. Збереження проєкту не перекодовує нічого. Відео копіюється один раз, а
далі переписується лише хвіст: кілобайти, а не гігабайти. Кінцівка файла
(`LHVEND` і зсув блока) дає знайти хвіст без розбору MP4. Версія проєкту — 1.1
(з 4.8.0): ті самі нові поля позначок, що й у `.lhshot` 1.2. Тестовий харнес
перевіряє: відео в проєкті байт у байт те саме,
повторне відкриття відновлює частини й позначки, хвіст переписується на місці,
поки відео відкрите.

### Кадрувати й зменшити відео (з 4.7.1)

Інструмент «Кадрувати» (`C`) працює і у відео: поставте на паузу, обведіть
потрібну ділянку й підтвердьте. Кадр один на все відео, полотно справді меншає,
як у знімків. «Розмір відео…» у панелі властивостей зменшує відео до потрібної
ширини чи висоти (пропорції зберігаються, збільшувати не можна). Рядок «Після
правок» показує, що вийде. І кадр, і розмір скасовуються `Ctrl+Z` разом з усім
іншим, зберігаються в проєкті `.lhvideo` і йдуть у «Зберегти як…» і «Копіювати
як файл». «Відкрити кадр як знімок» бере кадр у повній роздільності джерела,
лише обрізаний.

**Як.** Кадр і розмір живуть у координатах «документа» — відео, зменшеного до
поточного розміру. Розміри завжди парні (так вимагає H.264). Зменшення — усереднення
по площі, без перекодування до самого експорту: у проєкт іде лише блок `GEOM`.
На експорті кадр вирізається й масштабується перед позначками, тож позначки
лягають рівно туди, де їх намалювали.

### Експорт у GIF (з 4.7.1)

У списку «Зберегти» — «Експортувати в GIF…». Діалог компактний: кадрів за
секунду (5–30, не більше, ніж у джерела), ширина в пікселях (висота
пропорційна; типово не ширше 1280), кольорів (256/128/64/32), дизеринг, повтор
безкінечно чи один раз. Фрагмент — обрізанням на таймлайні, як для MP4:
вирізане й кадр із розміром у GIF враховуються, позначки впечені. Налаштування
запам'ятовуються. Під полями — приблизний розмір файлу; він перераховується під
час зміни налаштувань, а поки рахується — «Рахую…». Якщо виходить понад 25 МБ,
діалог підкаже зменшити ширину чи частоту.

**Як.** Свій кодувальник: глобальна палітра медіанним поділом з дюжини проб по
всьому фрагменту, один слот під прозорість, дизеринг Флойда–Стейнберга за
бажанням, LZW із блоками по 255 байтів. Кожен наступний кадр записується лише
там, де змінився (решта прозора), однакові кадри просто продовжують затримку,
а остання затримка підганяється так, щоб сума збіглась із тривалістю
фрагмента. Оцінка розміру чесна: кожна проба — пара сусідніх вихідних кадрів,
перший дає «повний» кадр, другий — різницю, і саме так рахується весь файл.
Тестовий харнес розбирає отриманий GIF: кадри декодуються LZW, і номер, впечений
у синтетику, має йти кадр у кадр.

### Лог браузера під час запису (з 4.13.0)

Поки йде запис, Little Helpers може писати **лог DevTools** зі сторінки в Chrome чи
Edge — консоль, помилки, мережеві запити, навігації — прив'язаний до часу відео
(CAPS-83). Для цього потрібне розширення браузера: «Розширення…» на вкладці
«Відео» розкладає його з самої програми в `%LOCALAPPDATA%\Little Helpers\BrowserExtension`
і відкриває теку; далі `chrome://extensions` (у Edge — `edge://extensions`) →
«Режим розробника» → «Завантажити розпаковане» → ця тека. Прапорець «Писати лог
DevTools…» там само вмикає й вимикає функцію; поруч видно, чи розширення
під'єднане. Лог пишеться з **активної вкладки** і перемикається разом із нею.

Як це влаштовано:

- **Канал — WebSocket лише на 127.0.0.1** (порт 47650). Native Messaging, з
  якого починався план, тут не працює: браузер запускає хост звичайним процесом,
  а програма вимагає прав адміністратора. Сервер приймає тільки Origin нашого
  розширення — його ID фіксує ключ у `manifest.json`, веб-сторінка Origin не
  підробить. З мережевої адреси машини до порту не під'єднатись.
- **Синхрон — за системним годинником**, без жодних пінгів: консоль, винятки й
  Log у протоколі DevTools несуть час у мілісекундах епохи, мережа — `wallTime`
  запиту. Потік запису на першому кадрі фіксує той самий годинник, паузи
  (CAPS-101) віднімаються так само, як для кадрів і звуку. Перевірено: подія
  стає в часі відео з точністю ~1 мс.
- **Екран запізнюється, лог — ні.** Лог показує, коли виконався код сторінки;
  на відео наслідок видно в середньому через ~90 мс (кадр сторінки, композиція,
  захоплення). Це виміряно наскрізним тестом зі справжнім Chrome: колір
  сторінки змінюється у відео через 1–3 кадри після `console.log` у ту ж мить.
- **Що не пишеться ніколи:** заголовки й тіла запитів і відповідей (зокрема
  `Authorization` і `Cookie`). Адреси — повністю; секрети в них ховає експорт
  звіту.
- Лог лежить поруч із записом у `.lhmeta` (блок `DEVT`) і в проєкті `.lhvideo`.
  У редакторі відео — мітки на доріжці позначок: помилки червоні, попередження
  бурштинові, навігації сині, мережа сіра; у «Відомостях» — кількість подій.
- Поки розширення під'єднане до вкладки, браузер показує смугу «розширення
  налагоджує цей браузер» — це обмеження платформи. У запис вона потрапляє, лише
  коли пишеться все вікно або ділянка екрана; запис із розширення «лише сторінки»
  (нижче) її не містить.

### Запис вікна браузера з розширення (з 4.14.0)

Запис можна почати просто з браузера (CAPS-107): клік по значку розширення →
**«● Записати це вікно»**. Пишеться саме це вікно — як «Клік по вікну: слідувати
за ним», навіть якщо в налаштуваннях вибрано «ділянкою». Прапорець **«Лише
сторінка»** (типово ввімкнено, запам'ятовується) відрізає вкладки, адресний рядок і
смугу відладчика: у відео — тільки вміст сторінки. Поки йде запис, на значку —
тривалість (червона; на паузі — жовта), а **клік по значку зупиняє запис**;
пауза, продовження й стоп — ще й у меню правої кнопки. Клавішу для старту й стопу
можна призначити в `chrome://extensions/shortcuts`. Зупинка звідки завгодно
(клавіша, плашка біля рамки, трей, закрите вікно) повертає значок до звичайного.
На вкладці «Відео» прапорець **«Дозволити розширенню починати й зупиняти запис»**
(типово ввімкнено) вимикає цю можливість, не вимикаючи лог.

Як це влаштовано:

- **Яке вікно — не знає ніхто, крім заголовка.** Розширення бачить вкладки й межі
  вікна в своїх одиницях, а дескриптора вікна Windows не має. Тож на мить воно
  дописує в заголовок сторінки мітку «⏺ LH-…» (до заголовка вікна вона доходить
  за ~200 мс); Little Helpers знаходить вікно з нею, каже «знайшов», розширення
  мітку прибирає, і лише коли її вже немає в заголовку, запис починається — у кадр
  вона не потрапляє. Мітку дописує скрипт із дозволом `activeTab`: його дає сам
  клік по значку, тож розширенню не треба доступу до всіх сайтів. На сторінки, куди
  скриптам зась (`chrome://…`, магазин розширень), мітку не поставити — тоді запас:
  вікно процесу браузера з тим самим заголовком вкладки, а кілька таких — ще й за
  межами вікна. Неоднозначно — помилка у вікні розширення, а не запис не того
  вікна. Сам клас вікна `Chrome_WidgetWin_1` нічого не доводить: його мають і
  програми на Electron (VS Code).
- **Лише сторінка** — це дочірнє вікно `Chrome_RenderWidgetHostHWND`: у Chrome і
  Edge воно рівно по області сторінки (перевірено на живому Chrome: 1084×705 при
  `innerWidth` × `innerHeight` = 1084 × 705). Кадр обрізається шейдером ще під час
  запису, тож MP4 у бібліотеці вже без інтерфейсу браузера. Область
  перечитується перед кожним кадром: відкрили DevTools, змінили розмір вікна чи
  перейшли на іншу вкладку — кадр іде за сторінкою (розмір відео — з першого
  кадру, нова область вписується в нього без спотворень).
- **Смуга відладчика — до першого кадру.** Коли лог увімкнено, розширення
  під'єднує відладчик ще до старту, тож смуга «налагоджує браузер» з'являється і
  зсуває сторінку до запису, а не посеред нього.
- **Той самий канал**, що й для лога: WebSocket на 127.0.0.1, лише Origin нашого
  розширення. Нові команди — `rec`, `stop`, `pause`, `resume`, `state`; усе інше
  ігнорується. Стан запису Little Helpers розсилає на кожну зміну, тож значок у
  кожному під'єднаному браузері каже правду.
- Перевірки: імітація розширення й «вікна браузера» (протокол, «лише сторінка»,
  стеження за областю) і наскрізний тест зі справжнім Chrome (значок, пауза, стоп,
  кути кадру в кожному кадрі — сторінка, лог із відладчиком, під'єднаним до
  старту, запас без мітки на `chrome://version`).

### Звіт для розробника (з 4.13.0)

«Зберегти ▾ → Звіт для розробника…» у редакторі відео (CAPS-84) збирає одне ціле
для баг-репорту: відео редакції (з обрізанням і позначками) і лог, перерахований у
її час. Діалог показує, що потрапить у звіт (тривалість, приблизний розмір, скільки
подій, помилок і кліків, які домени), і пропонує три формати:

- **Файл `.lhreport`** — ZIP без стиснення: `meta.json`, `log.jsonl`, `clicks.json`,
  `report.html`, `video.mp4`. Подвійний клік відкриває його в Little Helpers
  (відео — новим записом у бібліотеці, з логом і кліками); переглядач у самому
  розширенні («Відкрити звіт…» у його меню) грає відео прямо з архіву, без
  розпакування й без обмеження розміру; а без обох — досить перейменувати файл
  на `.zip` і відкрити `report.html`.
- **Один файл HTML** — відео всередині (base64), до ~100 МБ: більші браузери
  тримають погано, тож для великих записів цей варіант недоступний.
- **Тека** — те саме, що в архіві, окремими файлами.

Звіт — одна сторінка без залежностей: ліворуч відео, під ним стрічка з мітками
(помилки, попередження, мережа, навігації, кліки), праворуч журнал із фільтрами й
пошуком. Поточна подія підсвічується, пізніші — приглушені, журнал сам
прокручується за відео; клік по рядку перемотує відео на подію й розгортає
подробиці. Клавіші: пробіл — пуск/пауза, `N`/`P` — наступна/попередня подія, `E` —
наступна помилка. Мова — мова програми, тема — системна.

**Приватність.** «Приховати секрети в адресах» (типово ввімкнено) замінює на `•••`
значення параметрів, у назві яких є token, key, secret, password, auth, sig, session,
code тощо — у будь-якому полі, зокрема в тексті консолі. Решта даних сторінки
(тексти з консолі, адреси) лишається, про що діалог нагадує.

**Подвійний клік без UAC.** Програма працює з правами адміністратора, тож пряма
асоціація файлів питала б UAC на кожен подвійний клік. Натомість Little Helpers
реєструє `.lhreport`, `.lhvideo` і `.lhshot` для поточного користувача через тихий
передавач: `conhost --headless` запускає той самий exe без підвищення
(`__COMPAT_LAYER=RunAsInvoker`), а той лише передає шлях запущеній програмі
(`WM_COPYDATA`) і виходить — без вікон. Якщо програму не запущено, вона стартує
звичайно (один запит UAC) уже з цим файлом.

## Налаштування

- **Сторінки вкладок прокручуються** (з 4.9.0): коліщатко над сторінкою або
  тонка смуга праворуч, яка з'являється лише коли вміст довший за сторінку;
  `Tab` на контрол поза видимою частиною прокручує до нього. Вікно від цього не
  росте, і нова опція на будь-якій вкладці більше не впирається в підвал.
  З 4.13.1 (CAPS-106) кожна вкладка — окреме вікно-контейнер, а контроли
  живуть на «полотні» всередині нього; прокрутка просто зсуває полотно, і все,
  що за межами сторінки, обрізає сама система. Раніше контроли були дітьми
  головного вікна й обрізалися регіонами: при прокрутці вони налазили на шапку
  й підвал і лишали «привидів». Прокрутка тепер зупиняється на останньому
  контролі, а не їде в порожнечу. У 4.13.2 (CAPS-108) виправлено радіокнопки:
  при перенесенні на полотно порядок контролів перевертався, і межі груп
  з'їжджали — вибір мови гасив тему, кольори кліку — частоту кадрів тощо.
  Тепер контроли переносяться у своєму порядку; заодно правильний і порядок `Tab`.
  З 4.11.0 (CAPS-103) вкладка «Відео» на 100 % вміщається без прокрутки
  («Показати теку» — у рядку заголовка «Де записи»), а смуга стоїть впритул до
  рамки сторінки: зазор кольору сторінки між темною доріжкою і рамкою читався в
  темній темі як світла лінія.
...
- `lilhelpers.exe` — сидить у треї. Лівий клік по іконці або повторний запуск
  ...
  «Перегляд», «Знімки», «Відео» і «Налаштування»). Правий клік — меню: «Оновити
  до X», коли є нова версія; «Зупинити запис» і «Пауза запису», поки йде запис;
  «Налаштування…»; підменю «Знімки» (екран, вікно, ділянка, з буфера, записати
  відео, редактор); «Вихід». Усе перемикається наживо, без перезапуску; вікно
  можна закрити — програма лишається в треї.
...
`lilhelpers.exe` (~1,4 МБ у релізній збірці MSVC, статичний CRT, без зовнішніх
DLL).
...
| Файл | Що це |
|---|---|
| `README.md`, `README.en.md` | цей опис українською та англійською |
| `lilhelpers.cpp` | весь код (розкладка, курсор, день/ніч, перегляд, знімки й редактор, запис і редактор відео, тема вікна, локалізація, оновлення) |
| `browser-extension/` | розширення Chrome/Edge (лог DevTools, CAPS-83; запис вікна з його значка, CAPS-107) і переглядач звіту (`report.*`, `viewer.*`, CAPS-84); вшите в exe як ресурси |
...
- Приховування у відео під час відтворення показане плашкою, а справжнє
  розмиття — лише на паузі й у збереженому файлі.
- Режим «слідувати за вікном» потребує
  Windows 10 1903 або новішої; на старіших вікно записується як ділянка, де
  воно було на старті. Згорнуте вікно кадрів не дає — у відео лишається
  останній кадр.
- «Зберегти» у проєкт `.lhvideo` відео не перекодовує, а готовий файл
  («Зберегти як…», «Копіювати як файл», GIF) — перекодовує. Звук у записі й
  обрізанні перевірено переважно синтетикою з мітками часу; зі справжніми
  пристроями перевірок поки менше.
...
- Записується монітор, на якому вибрано ділянку; повернутий на 90° монітор
  поки не підтримано.
- Формат файлів редактора (`.lhshot`, `.lhvideo`) ще росте: нові версії додають
  поля, старі файли при цьому відкриваються (див. про версії формату).
