...
Peek у PowerToys, але **без окремого рантайму**: уся фіча коштувала 28 КБ
(387 КБ → 415 КБ у релізній збірці), бо
...
- **Що показується.** Зображення (JPEG, PNG, GIF, BMP, TIFF, ICO) — через GDI+,
  з поворотом за EXIF, зменшена копія кешується під розмір вікна; **анімовані
  GIF програються**, кожен кадр зі своєю затримкою. Текст і код
...
- **SVG показується розміткою, а не картинкою** — і це свідомо. Очевидний шлях,
  системний Direct2D (`ID2D1DeviceContext5::CreateSvgDocument`), перевірено
  пробою на двох справжніх файлах і відкинуто: SVG з `<mask>` малюється без
  маски, а `lilhelpers.svg` — власна іконка з Illustrator — виходить суцільною
  **чорною плямою**, бо заливки в ній задані CSS-класами в `<style>`, яких
  Direct2D не застосовує. Показати розмітку чесніше, ніж показати неправильне
  зображення. Вірний рендер SVG — етап 2, разом із PDF.
