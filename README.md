...
малювати (`Ctrl+Z` повертає й рамку). Унизу рейки — **«Копіювати»** (і `Enter`,
і `Ctrl+C`: скопіювати й закрити) та **«У вікно редактора»**, яка переносить
знімок разом із позначками в повне вікно з тоном, розміром, бібліотекою й
експортом. `Ctrl+S` — зберегти в бібліотеку й закрити. `Esc` знімає шари, як
...
**Рядок стану — три кнопки, і кожна робить головне одразу.** Ліворуч
**«Відкрити»** відкриває бібліотеку своїх знімків; у її списку — «Імпорт…»
(чужа картинка з файлу), «З буфера обміну», «Знімок ділянки». Праворуч
...
Поруч квадратна **«Копіювати»** (`Ctrl+C`) — лише іконка. Після вдалого
