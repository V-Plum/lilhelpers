...
позначає захоплення, і системний курсор вимкнено: курсор малюємо самі. Перевірка
— `_design_preview/caps73/follow_test.ps1`. Тут кадри справжні, бо WGC бачить
вікна навіть із пісочниці. Форму харнеса рухають, перекривають, звужують,
згортають і закривають, а декодований MP4 показує в кожній фазі саме її кольори.
...
30/60 кадрів, якість («Менша», «Звичайна», «Висока»), курсор і кліки, звук, межа
...
олівець, напис, приховування, маркер, лічильник, штамп і зображення. Немає лише
кадрування. Поставте відео на паузу, виберіть інструмент і малюйте на кадрі.
...
(з 4.8.0): ті самі нові поля позначок, що й у `.lhshot` 1.2. Перевірка —
`_design_preview/caps73/lhvideo_test.ps1`: відео в проєкті байт у байт те саме,
...
Перевірка — `_design_preview/caps73/geom_test.ps1` з розбором GIF
(`gifcheck.py`): кадри декодуються LZW і номер, впечений у синтетику, має йти
кадр у кадр.
...
- Запис відео експериментальний. Вікно
  записується як ділянка, де воно було на старті.
- Збереження з правками перекодовує відео. Звук (запис і обрізання) перевірено
  синтетикою з мітками часу; живий запис зі справжніми пристроями — на
  перевірці власника.
