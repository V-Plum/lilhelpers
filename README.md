...
**«Відкрити»** (`Ctrl+O`) в рядку стану бере зображення з файлу, а список поруч —
із буфера обміну або зі свіжого знімка ділянки. Масштаб перегляду там-таки:
повзунок, поточний відсоток і кнопка «100 %», яка показує знімок піксель у піксель. На час знімка редактор ховається, щоб не
