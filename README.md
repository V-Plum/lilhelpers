...
- `lilhelpers.exe` — сидить у треї. Лівий клік по іконці або повторний запуск
  ...
  «Налаштування»). Правий клік — меню з «Вихід». Усе перемикається наживо,
...
Апдейтер capslang ≤1.6.0 завантажує з релізу файл саме з ім'ям `capslang.exe`
(GitHub редіректить перейменований репозиторій, але не перейменований ассет),
тому кожен реліз поки що несе ті самі підписані байти під обома іменами —
`lilhelpers.exe` і `capslang.exe`. Ключ підпису не мінявся, тож стара версія
приймає новий бінарник як свій.

При першому старті нова версія переносить три сліди старої назви, кожен —
лише якщо він є:

1. **Файл.** Запущена як `capslang.exe` (так її записав старий апдейтер), вона
   копіює себе в `lilhelpers.exe` поруч, перезапускається з нього
   (`--after-rename <pid>`) і прибирає `capslang.exe` та `capslang.exe.old`.
   Відкат на 1.6.0 після цього навмисно неможливий: стара версія під новим
   ім'ям файла жила б із чужою гілкою реєстру й без задачі автозапуску.
2. **Реєстр.** `HKCU\Software\capslang` → `HKCU\Software\lilhelpers` (усі
   значення; старий ключ видаляється).
3. **Задача автозапуску** `capslang` → `lilhelpers`, уже з новим шляхом exe.

Якщо capslang ще працює поруч (нову версію запустили вручну), їй надсилається
«Вихід» — два перехоплювачі Caps Lock одночасно не потрібні.
...
capslang v1.0.0 як `Trojan:Win32/Wacatac.B!ml` (типове «сміттєве відро» хибних
...
| Файл | Що це |
|---|---|
| `lilhelpers.cpp` | весь код (розкладка, курсор, день/ніч, тема вікна, оновлення, перехід із capslang) |
