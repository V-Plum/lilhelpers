...
Що поки не зроблено і буде далі: решта інструментів (текст, стрілка, лінія,
олівець, овал, маркер, приховування ділянки, лічильник кроків, штампи),
кадрування, ручна правка експозиції та гами, відкриття збережених файлів.
...
- Редактор знімків експериментальний і поки має лише прямокутник: решта
  інструментів попереду.
- **Гілку HDR написано за специфікаціями, але на HDR-моніторі ще не
  перевірено.** Якщо знімок виходить не тим, що видно на екрані, — це саме
  сюди.
