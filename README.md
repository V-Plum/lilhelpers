...
**«Зберегти»** (`Ctrl+S`) кладе знімок **разом із позначками** у власну
бібліотеку — теку в `%LOCALAPPDATA%\Little Helpers\Library`. Це не картинка:
у файлі живуть оригінал, усі позначки з їхніми параметрами, кадр, тон і
геометрія, вкладені зображення й нумерація лічильників. Відкрив — і далі рухаєш
ту саму стрілку. Повторне натискання **перезаписує** той самий запис, а не
плодить копії; новий знімок починає новий запис. «Експорт» (`Ctrl+Shift+S`)
лишився тим, чим був, — PNG чи JPG для тих, кому потрібна просто картинка.
