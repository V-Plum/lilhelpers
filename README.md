...
- **Перегляд** — пробіл у Провіднику показує виділений файл, як Quick Look на
  macOS або Peek у PowerToys: зображення й SVG, відео кадром, моделі STL,
  текст, docx і форматований JSON. ⚠ Експериментально, з 2.2.0.
...
силами або системними API, які вже є у Windows — жодної нової залежності.

| Модель STL | SVG |
|---|---|
| ![Перегляд STL](screenshot-peek-stl.png) | ![Перегляд SVG](screenshot-peek-svg.png) |
| **Текст і код** | **Картка файлу** |
...
- **Текст і код** до 1 МБ — з визначенням кодування: BOM, далі сувора перевірка
  ...
  відомих текстових розширень — системне ANSI. Проза малюється пропорційним
  шрифтом, код — моноширинним.
...
- **SVG малюється системним Direct2D** (`ID2D1DeviceContext5::CreateSvgDocument`)
  ...
  елементами ми не малюємо взагалі, показуємо розмітку, і підпис каже чому.
  Препас лікує два місця, де реальні файли ламають Direct2D: Illustrator задає
  заливки CSS-класами в `<style>` (без препасу власна іконка проєкту виходила
  суцільною **чорною плямою**), а `<use href>` з SVG 2 Direct2D не бачить —
  впізнає лише старий `xlink:href`.
...
| `screenshot-peek-*.png` | вікно перегляду: STL, SVG, текст, картка файлу |
