...
Групу можна правити цілою: тогл перед палітрою вмикає режим, у якому колір,
розмір і прозорість ідуть усім кружечкам групи, а подвійний клік по кольору чи
розміру робить те саме разово, не вмикаючи режиму. «Мінус у кружечку» в кінці
...
**«Відкрити»** (`Ctrl+O`) в рядку стану бере зображення з файлу — PNG, JPEG, BMP,
GIF, TIFF, WebP, а також HEIC і AVIF, якщо в системі стоять їхні кодеки, а список поруч —
...
**«Поділитися»** в рядку стану відкриває **системне** меню поширення Windows —
те саме, що в Провіднику: цілі бере система з налаштувань користувача, власних
інтеграцій із месенджерами в програмі немає. Знімок їде через тимчасовий файл
у `%TEMP%`, і той прибирається за собою. Кнопки немає там, де система меню не
віддає: під адміністратором брокер поширення може бути недоступний.

У пакет кладуться **обидва формати** — і сам файл, і зображення. Одні цілі
(месенджери, пошта) беруть картинку, інші (редактори, Провідник) — файл; той,
хто вміє лише щось одне, інакше отримував порожнечу.

⚠ **Поширення вимагає окремого дозволу.** Програма запускається з правами
адміністратора (це потрібно для перехоплення CapsLock), а меню поширення й самі
приймачі — ні, і COM просто не пускає їхніх викликів «знизу вгору»: Windows
відкриває меню, але даних у програми **не питає** — приймач отримує порожнечу.
Це підтверджено вимірюванням, а не здогадкою.

Лікується чекбоксом **«Дозволити меню поширення діставати до програми»** на
вкладці «Знімки». За замовчуванням він **вимкнений**, бо відкриває виклики до
елевейтованого процесу звичайним процесам — вмикати його варто лише тому, кому
справді потрібне саме поширення; діє після перезапуску. Мовчазним збій більше не
буває: якщо меню відкрилось, а даних ніхто не спитав, програма каже про це прямо
й показує, що робити. Без дозволу надійні шляхи ті самі — «Копіювати» або
«Експорт».
...
так само; заміна всього знімка лишилась окремим пунктом у списку біля
